...
var to_node_override := false
var to_port_id := 0
var to_node_name := ""

var from_node_override := false
var from_port_id := 0
var from_node_name := ""

func _on_add_node_index_pressed(index: int) -> void:
	var n = graph_nodes[index].scene.instantiate() as QuestGraphNode
	quest_graph_editor.add_node(n, quest_graph_editor.get_local_mouse_position())

	if from_node_override:
		quest_graph_editor._on_connection_request(n.name, 0, from_node_name, from_port_id)
		from_node_override = false
	elif to_node_override:
		quest_graph_editor._on_connection_request(to_node_name, to_port_id, n.name, 0)
		to_node_override = false
...
func _on_connection_from_empty(to_node: StringName, to_port: int, release_position: Vector2) -> void:
	from_node_override = true
	from_port_id = to_port
	from_node_name = to_node
	add_node_popup_menu.popup_on_parent(Rect2(get_global_mouse_position(), Vector2.ZERO))


func _on_connection_to_empty(from_node: StringName, from_port: int, release_position: Vector2) -> void:
	to_node_override = true
	to_port_id = from_port
	to_node_name = from_node
	add_node_popup_menu.popup_on_parent(Rect2(get_global_mouse_position(), Vector2.ZERO))
